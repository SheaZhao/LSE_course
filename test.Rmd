...
```r
phf_hist <- ggplot(Women_2, aes(phf)) + geom_histogram()
```
```{r , echo=FALSE}
plot(phf_hist)
```
...
```r
age_hist <- ggplot(Women_2, aes(age50)) + geom_histogram()
```
```{r , echo=FALSE}
plot(age_hist)
```
...
```r
bargraph <- ggplot(Women_2, aes(x = occ, y = phf, fill = occ)) + geom_bar(stat = "identity")
```
```{r , echo=FALSE}
plot(bargraph)
```
...
F5.plot <- ggplot(first_5, aes(x=age50, y=phf, group = id, colour = id)) +
    geom_line()
