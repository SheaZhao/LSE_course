...
occ_phf.plot <- ggplot(Women_2, aes(x = occ, y = phf, group = occ, fill = occ)) + 
...
# PHF <- first_5$phf
# 
# plot(age, PHF, type = "p", pch = 16, col = rep(cols))
# plot(age, PHF, type = "l", pch = 16, col = rep(cols))
# plot(age, PHF, type = "b", pch = 16, col = rep(cols))
...
library(ggplot2)
p <- ggplot(first_5, aes(x = phf, y = age50, group = occ, color = occ)) +
    geom_point(size=3) +
